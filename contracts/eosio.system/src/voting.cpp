                                        ...
                                        uint16_t                              location)
{
   ...
   print(prod->can_return_slot);
   print(block_timestamp(eosio::current_time_point()).slot);
   if (prod->can_return_slot > block_timestamp(eosio::current_time_point()).slot) {
      print("블록 생성 불가능 상태");
      return;
   }
   ...
   if (prod != _producers.end()) {
      ...
      if (prod2 == _producers2.end()) {
         ...
         // When introducing the producer2 table row for the first time, the producer's votes must also be accounted for
         // in the global total_producer_votepay_share at the same time.
      }
   } else {
                                   ...
                                   uint16_t                              location)
{
   ...
   std::visit([&](auto&& auth) { check(auth.is_valid(), "invalid producer authority"); }, producer_authority);
...
void system_contract::unregprod(const name& producer)
{
   ...
   _producers.modify(prod, same_payer, [&](producer_info& info) { info.deactivate(); });
}
...
void system_contract::update_elected_producers(const block_timestamp& block_time)
{
   ...
   std::string totalActiveVotedProds_string = std::to_string(totalActiveVotedProds);
   const char* totalActiveVotedProds_value  = totalActiveVotedProds_string.c_str();
   //jjy_log("totalActiveVotedProds");
   //jjy_log(totalActiveVotedProds_value);
   std::vector<producer_location_pair> active_producers, top_producers;
   active_producers.reserve(totalActiveVotedProds);

   for (auto it = idx.cbegin(); it != idx.cend() && active_producers.size() < totalActiveVotedProds /*TELOS*/ &&
                                0 < it->total_votes && it->active();
        ++it) {
      //jjy_log("it_producer_name : ");
      //std::string str            = it->owner.to_string();
      //const char* value_producer = str.c_str();
      //jjy_log(value_producer);
      if (it->is_active) {
         active_producers.emplace_back(
            eosio::producer_authority{.producer_name = it->owner, .authority = it->get_producer_authority()},
            it->location);
      
      }
         }
   //jjy_log("vote_for_end");
   ...
   // TELOS END
  // jjy_log("top_producers");
   std::sort(top_producers.begin(), top_producers.end(),
             [](const producer_location_pair& lhs, const producer_location_pair& rhs) {
   ...
   //jjy_log("top_producers_sort");
   std::vector<eosio::producer_authority> producers;
   std::string                            top_producers_string = std::to_string(top_producers.size());
   const char*                            top_producers_value  = top_producers_string.c_str();
   //jjy_log(top_producers_value);
   producers.reserve(top_producers.size());
   for (auto& item : top_producers)
      producers.push_back(std::move(item.first));
   //jjy_log("top_producers_push_back");
   // TELOS BEGIN
   auto schedule_version = set_proposed_producers(producers);
   //jjy_log("set_proposed_producers");
   if (schedule_version >= 0) {
      print("\n**new schedule was proposed**");

      _gstate.last_proposed_schedule_update = block_time;

      _gschedule_metrics.producers_metric.erase(_gschedule_metrics.producers_metric.begin(),
      ...
      //jjy_log("producers_metric");
      _gstate.last_producer_schedule_size =
         static_cast<decltype(_gstate.last_producer_schedule_size)>(top_producers.size());
   }
   // TELOS END
   //jjy_log("top_producers_end");
}

double stake2vote(int64_t staked)
{
   /// TODO subtract 2080 brings the large numbers closer to this decade
   double weight = int64_t((current_time_point().sec_since_epoch() - (block_timestamp::block_timestamp_epoch / 1000)) /
                           (seconds_per_day * 7)) /
                   double(52);
 ...
 * The maximum weighted vote will be reached if an account votes for the maximum number of registered producers (up to
 * 30 in total).
 */
                                                   ...
                                                   double            shares_rate_delta)
{
   ...
   if (ct > _gstate3.last_vpay_state_update) {
      delta_total_votepay_share =
         _gstate3.total_vpay_share_change_rate * double((ct - _gstate3.last_vpay_state_update).count() / 1E6);
   }
                                                      ...
                                                      bool                                    reset_to_zero)
{
   ...
   if (shares_rate > 0.0 && ct > prod_itr->last_votepay_share_update) {
      delta_votepay_share =
         shares_rate * double((ct - prod_itr->last_votepay_share_update).count() / 1E6); // cannot be negative
   }
...
void system_contract::voteproducer(const name& voter_name, const name& proxy, const std::vector<name>& producers)
{
   ...
   */
   ...
   if( rex_itr != _rexbalance.end() && rex_itr->rex_balance.amount > 0 ) {
      check_voting_requirement( voter_name, "voter holding REX tokens must vote for at least 21 producers or for a
   proxy" );
   }
   */
}
...
void system_contract::voteupdate(const name& voter_name)
{
   ...
   if (voter->staked != new_staked) {
      ...
      _voters.modify(voter, same_payer, [&](auto& av) { av.staked = new_staked; });
   }
                                   ...
                                   bool                     voting)
{
   ...
   // print("\n Voter : ", voter->last_stake, " = ", voter->last_vote_weight, " = ", proxy, " = ", producers.size(), " =
   // ", totalStaked, " = ", new_vote_weight);
   ...
   if (voter->last_stake > 0) {
      ...
      if (voter->proxy) {
         ...
         _voters.modify(old_proxy, same_payer, [&](auto& vp) { vp.proxied_vote_weight -= voter->last_stake; });
   ...
   if (proxy) {
      ...
      _voters.modify(new_proxy, same_payer, [&](auto& vp) { vp.proxied_vote_weight += voter->staked; });
...
void system_contract::regproxy(const name& proxy, bool isproxy)
{
   ...
   if (pitr != _voters.end()) {
      ...
      _voters.modify(pitr, same_payer, [&](auto& p) { p.is_proxy = isproxy; });
      // TELOS BEGIN
         ...
         for ( auto acnt : voter.producers ) {
            ...
            if ( prod2 != _producers2.end() ) {
               ...
               double new_votepay_share = update_producer_votepay_share( prod2,
                                             ...
                                             crossed_threshold && !updated_after_threshold // only reset votepay_share
   once after threshold
                                          );
   ...
   */
   ...
   if (voter.proxy) { // this part should never happen since the function is called only on proxies
      if (voter.last_stake != totalStake) {
         auto& proxy = _voters.get(voter.proxy.value, "proxy not found"); // data corruption
         _voters.modify(proxy, same_payer, [&](auto& p) { p.proxied_vote_weight += totalStake - voter.last_stake; });
...
void system_contract::recalculate_votes()
{
   if (_gstate.total_producer_vote_weight <= -0.1) { // -0.1 threshold for floating point calc ?
      ...
      for (auto producer = _producers.begin(); producer != _producers.end(); ++producer) {
         _producers.modify(producer, same_payer, [&](auto& p) { p.total_votes = 0; });
      }
