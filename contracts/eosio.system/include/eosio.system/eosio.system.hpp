...
struct [[eosio::table("global"), eosio::contract("eosio.system")]] eosio_global_state : eosio::blockchain_parameters
{
   ...
   EOSLIB_SERIALIZE_DERIVED(
      eosio_global_state,
      eosio::blockchain_parameters, (max_ram_size)(total_ram_bytes_reserved)(total_ram_stake)(last_producer_schedule_update)(last_proposed_schedule_update)(last_pervote_bucket_fill)(pervote_bucket)(perblock_bucket)(total_unpaid_blocks)(total_activated_stake)(thresh_activated_stake_time)(last_producer_schedule_size)(total_producer_vote_weight)(last_name_close)(block_num)(last_claimrewards)(next_payment)(new_ram_per_block)(last_ram_increase)(last_block_num)(total_producer_votepay_share)(revision))
};
...
struct [[eosio::table, eosio::contract("eosio.system")]] producer_info
{
   ...
   uint32_t punished_round = 0;
   uint32_t total_punished = 0;
   uint32_t can_return_slot;
   ...
   void kick(kick_type kt, uint32_t penalty = 0)
   {
      ...
      std::string name_str = owner.to_string();
      ...
      if (penalty == 0)
         ...
         can_return_slot=block_timestamp(eosio::current_time_point()).slot+(200*times_kicked);
          //object_log("block_timestamp(eosio::current_time_point()).slot",block_timestamp(eosio::current_time_point()).slot);
         //object_log("last_time_kicked.slot",last_time_kicked.slot);
         //object_log("can_return_slot",can_return_slot);
      switch (kt) {
      case kick_type::REACHED_TRESHOLD:
         kick_reason_id = uint32_t(kick_type::REACHED_TRESHOLD);
         kick_reason =
            "Producer account was deactivated because it reached the maximum missed blocks in this rotation timeframe.";
         break;
   ...
   {
      ds << t.owner << t.total_votes << t.producer_key << t.is_active << t.unreg_reason << t.url << t.unpaid_blocks
         << t.lifetime_produced_blocks << t.strike_count << t.punished_round << t.total_punished << t.can_return_slot << t.missed_blocks_per_rotation << t.lifetime_missed_blocks << t.last_claim_time
         << t.location << t.kick_reason_id << t.kick_reason << t.times_kicked << t.kick_penalty_hours
         << t.last_time_kicked;
   ...
   {
      return ds >> t.owner >> t.total_votes >> t.producer_key >> t.is_active >> t.unreg_reason >> t.url >>
             t.unpaid_blocks >> t.lifetime_produced_blocks >> t.strike_count >> t.punished_round  >> t.total_punished >> t.can_return_slot >> t.missed_blocks_per_rotation >>
             t.lifetime_missed_blocks >> t.last_claim_time >> t.location >> t.kick_reason_id >> t.kick_reason >>
             t.times_kicked >> t.kick_penalty_hours >> t.last_time_kicked >> t.producer_authority;
   }
};
...
struct [[eosio::table, eosio::contract("eosio.system")]] voter_info
{
   ...
   EOSLIB_SERIALIZE(
      voter_info,
      (owner)(proxy)(producers)(staked)(last_stake)(last_vote_weight)(proxied_vote_weight)(is_proxy)(flags1)(reserved2)(reserved3))
};
...
public:
    ...
    */
   [[eosio::action]] void rentcpu(const name& from, const name& receiver, const asset& loan_payment,
    ...
    *    Loan balance represents a reserve that is used at expiration for automatic loan renewal.
    */
   [[eosio::action]] void rentnet(const name& from, const name& receiver, const asset& loan_payment,
                                  const asset& loan_fund);
    ...
    */
   [[eosio::action]] void buyrambytes(const name& payer, const name& receiver, uint32_t bytes);
    ...
    */
   [[eosio::action]] void sellram(const name& account, int64_t bytes);
    ...
    */
   [[eosio::action]] void regproducer(const name& producer, const public_key& producer_key, const std::string& url,
                                      uint16_t location);
    ...
    * @pre Authority of producer to register
    */
   [[eosio::action]] void regproducer2(const name& producer, const eosio::block_signing_authority& producer_authority,
    ...
    */
   [[eosio::action]] void limitauthchg(const name& account, const std::vector<name>& allow_perms,
   ...
   // TELOS BEGIN
   [[eosio::action]] void unregreason(const name& producer, std::string reason);

   [[eosio::action]] void votebpout(name bp, uint32_t penalty_hours);

   [[eosio::action]] void setpayrates(uint64_t inflation, uint64_t worker);

   [[eosio::action]] void distviarex(name from, asset amount);
...
private:
   ...
   void check_voting_requirement(
      const name& owner, const char* error_msg = "must vote for at least 21 producers or for a proxy before buying REX")
      const;
   ...
   template <typename T>
   int64_t rent_rex(T & table, const name& from, const name& receiver, const asset& loan_payment,
                    const asset& loan_fund);
   template <typename T>
   ...
   void changebw(name from, const name& receiver, const asset& stake_net_quantity, const asset& stake_cpu_quantity,
                 bool transfer);
   ...
   double update_producer_votepay_share(const producers_table2::const_iterator& prod_itr, const time_point& ct,
                                        double shares_rate, bool reset_to_zero = false);
   double update_total_votepay_share(const time_point& ct, double additional_shares_delta = 0.0,
   ...
   void adjust_resources(name payer, name account, symbol core_symbol, int64_t net_delta, int64_t cpu_delta,
                         bool must_not_be_managed = false);
   void process_powerup_queue(time_point_sec now, symbol             core_symbol, powerup_state & state,
   ...
   void add_to_blockinfo_table(const eosio::checksum256& previous_block_id, const eosio::block_timestamp timestamp)
      const;
   ...
   std::vector<producer_location_pair> check_rotation_state(std::vector<producer_location_pair> producers,
